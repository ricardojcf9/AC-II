 ...
 main:
 	...
 	sw	$t1, TRISE($s0)		# WRITE (Write TRISB register)
 	
 	lw	$t1, TRISB($s0)		# READ  (Mem_addr = 0xBF880000 + 0x6040)
 	andi	$t1,$t1,0xF		# MODIFY (bit0 = bit1 = bit2 = bit3 = 1 (1 means INPUT))
 	sw	$t1, TRISB($s0) 	# WRITE (Write TRISB register)
 	
while:					# while(1) {
	li	$a0,400
	jal	delay			# delay(400)
	beq	$s1,0,else		# if (counter != 0)
	...
	or	$t0,$t0,$s1		# LATE[3..0] = counter
	sw	$t0, LATE($s0)		# WRITE
	j	while			# }


 delay:
	ble	$a0,0, endFor 		# for (: ms > 0; ms--)
	li	$v0, resetCoreTimer	#
	syscall				# resetCoreTimer();
 while2:
	li	$v0, readCoreTimer	# while(readCoreTimer())
	syscall				#
